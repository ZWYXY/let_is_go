...
func (this *Server) StartServer() {
	...
	// 开启一个goroutine 定时打印OnlineMap中的用户信息
	go func(server *Server) {
		for {
			time.Sleep(time.Second * 10 * 60)
			onlineMap := server.OnlineMap
			for _, value := range onlineMap {
				fmt.Println("当前在线用户人数" + fmt.Sprintf("%d", len(onlineMap)) +
					"，在线用户信息{" + value.Addr + "," + value.Name + "}")
			}
			fmt.Println("-----------------------")
		}
	}(this)
...
func (this *Server) BusinessHandler(conn net.Conn) {
	...
	for {
		select {
		case <-alive:
			// 什么都不做，等时间到了，下面这个time.After会有消息出来，下面这个case就被执行
		case <-time.After(time.Second * 10 * 60):
			// 通知用户下线
			user.SendMessage("检测到长时间未通信，连接关闭。")
			// 用户下线
			user.Offline()
			// 关闭客户端服务端连接
			conn.Close()
