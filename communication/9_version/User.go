...
func (this *User) DealMessage(msg string) {
	...
	if msg == "who" {
		...
		this.server.mapLock.Unlock()
	} else if len(msg) > 7 && "rename|" == msg[:7] { // 修改昵称功能
	...
	} else if len(msg) > 3 && msg[:3] == "to|" { // 私聊功能 to|IP:PORT|content
		content := strings.Split(msg, "|")[2]
		remoteAddr := strings.Split(msg, "|")[1]
		remoteUser, ok := this.server.OnlineMap[remoteAddr]
		if !ok {
			this.SendMessage("不存在的用户")
		} else {
			remoteUser.SendMessage(this.Addr + ":\n" + this.Name + ":" + content)
		}
	} else {
		// 通过服务端广播，接收到的消息
