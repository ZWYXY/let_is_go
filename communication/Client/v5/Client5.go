...
// 私聊模式
func (client *MyClient) SendToSpecial() {
	var msg string
	fmt.Scanln(&msg)
	msgStringArray := strings.Split(msg, "|")
	ip := msgStringArray[0]
	content := msgStringArray[1]
	msgToSend := "to|" + ip + "|" + content
	client.ServerConn.Write([]byte(msgToSend))
}
